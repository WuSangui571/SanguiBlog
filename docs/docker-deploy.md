...
#   - 注意是否有 ERROR/报错行，尤其是 ALTER TABLE 相关输出（详见 13.8 节）
...
- 若 AI 表不完整但核心业务表正常且 `AI_RAG_ENABLED=false`：可接受，AI 功能本已关闭，不影响博客上线。后续如需启用 AI，参考第 14 节"AI 表诊断与修复"。
