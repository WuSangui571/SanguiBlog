...
启动成功后访问 `http://localhost:8090` 即可看到博客首页（默认 WEB_PORT=8090，避免与宿主机 Nginx 80/443 冲突）。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `WEB_PORT` | `8090` | 宿主机端口。默认 8090 避免与宿主机 Nginx 80/443 冲突 |
| `JAVA_TOOL_OPTIONS` | `-Duser.timezone=Asia/Shanghai -Xms256m -Xmx512m` | JVM 堆内存限制 |
...
> **迁移说明**：`AI_DASHSCOPE_API_KEY` 已废弃，请改用 `AI_OPENAI_API_KEY` + `AI_OPENAI_BASE_URL`。
...
```bash
docker compose -f docker-compose.prod.yml exec web sh -c 'getent hosts api.openai.com || nslookup api.openai.com || true'
```
...
docker run --rm --network sanguiblog_sanguiblog-net alpine:3.20 sh -c '
  ...
  curl -I --connect-timeout 5 https://api.openai.com
'
```

> 如果使用第三方 OpenAI-compatible provider（如 DashScope 兼容模式），请将上面的 `api.openai.com` 替换为对应的 `AI_OPENAI_BASE_URL` 域名。

**检查 AI API Key 是否注入（不打印密钥值）：**
...
当生产 RAG / AI provider 异常且已导致服务不可用时，可临时关闭 RAG 作为止血操作：
...
| AI 聊天无响应 | OpenAI API Key 未配 | 设置 `AI_OPENAI_API_KEY` 后重启 |
| RAG 不生效 | PgVector 未初始化 | 检查 `AI_RAG_PGVECTOR_INITIALIZE_SCHEMA=true` |
| uploads 上传失败 | 上传卷子目录权限不一致（root:root 755 导致 backend 非 root 用户无法写入） | 本地 `docker-compose.yml` 和生产 `docker-compose.prod.yml` 均已内置 `uploads-init` 一次性权限修复服务；若 `uploads-init` 未能覆盖（如 `docker compose cp` 或 tar 解压后引入新的 root-owned 目录），backend 启动时 `StoragePathResolver` 会立即失败并提示修复命令；执行第 14.5 节写入探针和手动修复命令 |
| 容器重启后数据丢失 | 未使用 Docker volume | 确认 `docker-compose.yml` 中定义并使用了 volumes |
| 端口冲突 | 宿主机 80 端口被占用 | 设置 `WEB_PORT=8090` 在 `.env` 中（默认值） |
| AI 聊天报错"服务器内部错误"，日志提示 `Table 'sanguiblog_db.ai_chat_messages' doesn't exist` | MySQL 数据卷复用（AI 表 DDL 更新后未重新执行 init 脚本）| 手动导入缺失的 AI 表，见下方 [AI 表诊断与修复](#15-ai-表诊断与修复) |
| `down -v` 后首次启动，backend/web 状态为 unhealthy 或反复重启 | MySQL 健康检查仅验证端口连通，未等待 `sanguiblog_db.sql` 初始化完成就通知 backend 启动 | backend 使用 `restart: always`，schema 就绪后自动恢复 |
...
备用/临时服务器建议**关闭 AI 功能**，避免引入外部 AI provider 依赖和 PgVector/RAG 初始化风险。核心博客、管理端、上传、sitemap/robots 均不依赖 AI。
...
AI_RAG_SYNC_ON_STARTUP=false
# AI_OPENAI_API_KEY 留空
# AI_OPENAI_BASE_URL 留空
...
> 以下命令默认使用 `http://localhost:8090`。若设置了不同的 `WEB_PORT`，替换端口即可。
...
1. 浏览器访问 `http://localhost:8090`，点击导航栏或直接访问 `/admin/login`
2. 使用安装时创建的超级管理员账号登录
3. 确认能进入管理后台面板

**上传目录写入探针：**

`docker-compose.yml` 和 `docker-compose.prod.yml` 启动时都会先运行一次 `uploads-init` 服务，自动创建 `posts`、`covers`、`avatar`、`games`、`site/wechat` 子目录，并将 `uploads_data` 卷权限修正为 backend 运行用户可写。下面的探针用于确认权限是否符合预期。
...
```bash
docker compose exec -u root backend sh -c 'mkdir -p /data/uploads/posts /data/uploads/covers /data/uploads/avatar /data/uploads/games /data/uploads/site/wechat && chown -R sangui:sangui /data/uploads && chmod -R u+rwX,g+rwX /data/uploads'
```

修复后重新执行写入探针确认。造成该问题的常见原因是 named volume 初次挂载、`docker compose cp`、tar 解压或 root 用户手工复制文件时创建了 `root:root 755` 子目录；backend 容器以非 root 用户 `sangui` 运行，无法在这些子目录下继续创建日期目录。

**关于 `docker compose restart backend`：** 仅重启 backend 容器不会重新运行 `uploads-init` 服务（该服务是一次性任务，`restart: "no"`）。如果在 backend 运行期间通过 `docker compose cp` 或其他 root 操作引入了新的 root-owned 上传子目录，`docker compose restart backend` **无法**修复权限问题。此时 backend 的 `StoragePathResolver` 会在启动时立即抛出 `IllegalStateException`，消息中包含不可写路径及 `chown` 修复命令。支持的无手动 `chown` 部署命令是：
- 本地：`docker compose up -d --build`（会重新创建并运行 `uploads-init`）
- 生产：`docker compose -f docker-compose.prod.yml up -d`（会检测 `uploads-init` 状态并重新运行）
...
### 14.8 MySQL Init 风险观察

`sanguiblog_db.sql` 已做幂等处理：所有建表使用 `CREATE TABLE IF NOT EXISTS`，种子数据使用 `INSERT IGNORE` 或 `WHERE NOT EXISTS`，避免重复执行时误删或报错。旧版 `DROP TABLE IF EXISTS` 块已移除。脚本中保留 `ai_chat_sessions` 列的 INFORMATION_SCHEMA 兼容迁移过程，兼容 MySQL 8.0 与 MariaDB。
...
- 若 `SHOW TABLES LIKE 'ai_%'` 返回 6 张表且 backend 日志无 `doesn't exist`：通过。`sanguiblog_db.sql` 中的兼容列迁移应能在 MySQL 8.0 上直接执行，不应再出现 `ADD COLUMN IF NOT EXISTS` 语法错误。
- 若 AI 表不完整但核心业务表正常且 `AI_RAG_ENABLED=false`：可接受，AI 功能本已关闭，不影响博客上线。后续如需启用 AI，参考第 15 节"AI 表诊断与修复"。
...
| `/` 返回前端页面 | `curl -i http://localhost:8090/` → 200 + text/html | ☐ |
| `/api/site/meta` 返回 JSON | `curl -i http://localhost:8090/api/site/meta` → `"success":true` | ☐ |
| `/sitemap.xml` 返回 XML | `curl -i http://localhost:8090/sitemap.xml` → Content-Type: application/xml | ☐ |
| `/robots.txt` 返回 text/plain | `curl -i http://localhost:8090/robots.txt` → Content-Type: text/plain | ☐ |
| 管理端可登录 | 浏览器成功登录管理后台 | ☐ |
...
```bash
# 1. 确认 Spring AI OpenAI 密钥已注入（不要打印完整密钥）
...
# 2. 确认 AI 助手已启用
curl -s http://localhost:8090/api/site/meta | python3 -c "import sys,json; print(json.load(sys.stdin)['data']['aiAssistant'])"
