...
| uploads 上传失败 | 上传卷子目录权限不一致 | 当前 `docker-compose.prod.yml` 已内置 `uploads-init` 一次性权限修复服务；若旧部署仍失败，执行第 14.5 节写入探针和手动修复命令 |
...
`docker-compose.prod.yml` 启动时会先运行一次 `uploads-init` 服务，自动创建 `posts`、`covers`、`avatar`、`games`、`site/wechat` 子目录，并将 `uploads_data` 卷权限修正为 backend 运行用户可写。下面的探针用于确认权限是否符合预期。
