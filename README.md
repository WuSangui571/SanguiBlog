...
> Current site version: `V2.2.10` (provided centrally by backend `site.version`, and displayed on the homepage banner as `SANGUI BLOG // <version>`)
>
> `V2.2.x` continues the AI assistant system introduced in `V2.2.0`. The current `V2.2.10` site version keeps multi-turn chat for logged-in users, blog-article RAG, current article page context enhancement, super-admin knowledge import, and backend AI session auditing, while also updating the homepage and navigation visuals.
...
Starting from `V2.2.0`, the project includes a built-in AI assistant. The current `V2.2.10` site version continues and improves this capability. It includes:
