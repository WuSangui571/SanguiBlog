...
> 当前站点版本号：`V2.2.21`（统一由后端 `site.version` 提供，首页 Banner 展示为 `SANGUI BLOG // <version>`）
>
> `V2.2.x` 延续了 `V2.2.0` 引入的 AI 助理体系：当前 `V2.2.21` 继续沿用登录用户多轮会话、博客文章 RAG、当前文章页上下文增强、超级管理员知识库导入与后台 AI 会话审计，并包含近期首页移动端布局、重定向来源统计、文章区分阶段加载等体验优化。
...
`V2.2.0` 起，项目已内置 AI 助理；当前 `V2.2.21` 延续并完善了该能力。能力包括：
