...
> 当前站点版本号：`V2.2.7`（统一由后端 `site.version` 提供，首页 Banner 展示为 `SANGUI BLOG // <version>`）
>
> `V2.2.x` 延续了 `V2.2.0` 引入的 AI 助理体系：当前 `V2.2.7` 继续沿用登录用户多轮会话、博客文章 RAG、当前文章页上下文增强、超级管理员知识库导入与后台 AI 会话审计，并更新了首页与导航的新版视觉。
...
`V2.2.0` 起，项目已内置 AI 助理；当前 `V2.2.7` 延续并完善了该能力。能力包括：
