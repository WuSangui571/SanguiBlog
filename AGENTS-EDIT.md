...
## V1.1.50 (2025-11-23)
- **评论统计**: 后端 PostService 通过 commentRepository.countByPostIdAndStatus 统计已批准评论数量，确保首页文章列表的评论数真实可靠。
- **浏览计数**: 取消文章详情的只读事务，使浏览量更新可即时落库，防止页面刷新后数值不变。
- **版本**: 首页 Banner 更新为 SANGUI BLOG // V1.1.50。
