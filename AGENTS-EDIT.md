...
## V2.1.284 (2026-01-03)
- **模块/页面**：修复访问日志“文章访问”筛选错误：后端按“文章列展示口径”判断文章访问（以 `post.id` 是否存在为准），避免将普通页面/机器页面混入文章访问结果；同时修正“隐藏 robots/sitemap”仍可能出现误筛/漏筛的问题，统一以展示口径精确匹配 `robots.txt/sitemap.xml`。
