...
## V1.3.24 (2025-11-29)
- **后台导航**：管理侧边栏支持“概览与洞察 / 内容运营 / 账号与权限 / 系统配置”四个分组，保持原有路由与权限逻辑不变，仅在 UI 上以二级菜单形式展示，便于快速定位模块。
- **版本**：首页 Banner 更新为 `SANGUI BLOG // V1.3.24`。

## V1.3.25 (2025-11-29)
- **导航体验**：修复后台侧边栏无法滚动的问题，`nav` 元素新增 `min-h-0` 以在 Flex 布局下正确出现滚动条，确保底部菜单可被访问。
- **版本**：首页 Banner 更新为 `SANGUI BLOG // V1.3.25`。

## V1.3.26 (2025-11-29)
- **侧边栏滚动**：`aside` 增加 `overflow-hidden` 并固定在 `inset-y-0`，导航区域的 `min-h-0` + `overflow-y-auto` 现在能生效，滚轮只作用于左侧菜单，底部按钮可正常点击。
- **版本**：首页 Banner 更新为 `SANGUI BLOG // V1.3.26`。

## V1.3.27 (2025-11-29)
- **顶部遮挡**：后台侧边栏现在依据 `headerHeight` 计算 `top` 与 `height`，避免被全局导航覆盖或无法滚动，首个菜单项重新完整可见。
- **版本**：首页 Banner 更新为 `SANGUI BLOG // V1.3.27`。

## V1.3.28 (2025-11-29)
- **文章详情**：修复 props 不匹配导致文章长期停留在“加载中”的问题，ArticleDetail 统一接受 `articleData/commentsData` 并保留兼容逻辑，返回列表按钮立即滚动回顶。
- **后台仪表盘**：移除独立的“数据分析”页面，Dashboard 直接整合全部图表与实时访问日志，提供最近 7/14/30 天或自定义日期筛选、分页及超级管理员日志清理。
- **访问日志**：新增 `/api/admin/analytics/page-views` 分页接口，基于 IP 粗略推断地域并返回访客身份、来源、终端等字段，管理员日志面板现可按条件刷新查看。
