...
*   **站长工具验证（Bing）**：必应站长工具（Bing Webmaster Tools）验证采用 `msvalidate.01` 的 HTML `<meta>` 标记；该标记当前放置于 `SanguiBlog-front/index.html` 的 `<head>` 中（位于首个 `<body>` 之前）。注意：即使验证成功后也需要长期保留该 `<meta>` 标记，否则会失去“已验证”状态。
