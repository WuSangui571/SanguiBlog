...
*   `/admin/comments` ? `CommentsAdminView` ??????????????????????????????????? `/api/admin/comments` ???????????????? `postId` ?????
*   ???? `/api/admin/comments` ????
    *   `GET /api/admin/comments?postId=&status=&keyword=&page=&size=` ?? `PageResponse<AdminCommentItemDto>`???????Slug????IP?????????????????
    *   `PUT /api/admin/comments/{id}` ?? `{content,status}`??????????????????? `APPROVED/PENDING/REJECTED/SPAM`??
    *   `DELETE /api/admin/comments/{id}` ???????????????????????
*   `CommentService` ?? `searchComments` ? `updateCommentAsAdmin`???????????????????????????????????????
*   ?????????????????? `COMMENT_CREATE`?`COMMENT_REPLY`?`COMMENT_REVIEW`?`COMMENT_DELETE` ????????????? `PermissionNotice`???????
*   ??/?????? `/posts/{id}/comments`???? `AdminNoticeBar` ??????????????????????

### 3.10 ??????????
*   ?? `/api/admin/analytics/summary?days=<7|14|30>&top=<5>&recent=<30>` ???? `AdminAnalyticsController` ??????? `AnalyticsService.loadAdminSummary` ?? `analytics_page_views` ? `analytics_traffic_sources`??? `AdminAnalyticsSummaryDto`??? `overview`???/?? PV?UV???????????/?????`dailyTrends`????? PV/UV??`trafficSources`?`topPosts` ?? `recentVisits`??? 30 ? PV?????IP????referrer?Geo???????
*   ????? `DashboardView` ?? `AnalyticsSummaryContext` ?? Summary????? KPI ?????????????????????????????? `reload()` ?????
*   `/admin/analytics` ???? 7/14/30 ???????? PV?UV???????????????????????????????????????/?????????? URL?IP?Geo ???????????????????IP?????????????
*   `AnalyticsService.recordPageView` ???? `pageTitle/referrer/geo/userAgent` ??? `analytics_page_views`?????????????????????????????????????? `/api/admin/analytics/page-views/me` ???? PV??????????ï¼ˆV1.3.1 å¼€å§‹ä»…åœ¨ `pageTitle/referrer` åŒ…å« `admin` æ—¶ç¦»çº¦ SUPER_ADMIN æ—¥å¿—ï¼Œç®¡ç†äººå‘˜ä»¥å®¢ã€æ–‡ç« è§†è§ˆä¹Ÿä¼šè¢«å®Œæ•´è®°å½?ï¼‰
*   ???? PV ???? `updateTrafficSourceStat`??? referrer ???????????????????? `analytics_traffic_sources(stat_date, source_label, visits, percentage)`??????????????????

### 3.11 ????
*   `/admin/permissions` ? `PermissionsView` ????????????????????????????? `AdminNoticeBar + useTimedNotice` ???????
*   ?????? `/api/admin/permissions`?`PermissionService` ??? `PermissionDefinition` ??????????????????????/???????????????????????????
*   ????????/???????????????????? `adminUpdateRolePermissions(roleCode, permissions)`??????? `role_permissions` ????????
*   ???? `permissions_seed.sql`????????? `permissions` ? `role_permissions`???????? `PermissionDefinition` ?? `defaultRoles` ????????????????????
...
### 4.6 ?????????
*   `POST /api/analytics/page-view` ?? `PageViewRequest` ?????? IP?User-Agent?????? referrer??? `analytics_page_views`????????????????????????????????????????????
*   `AnalyticsService.updateTrafficSourceStat` ???? PV ????? `analytics_traffic_sources(stat_date, source_label, visits, percentage)`??????? referrer ????????????????????????????
*   `PostService.incrementViews` 每次成功 +1 阅读数后都会构造 `PageViewRequest` 并直接调用 `recordPageView`，即使前端埋点异常也能确保 `analytics_page_views/analytics_traffic_sources` 有真实数据。
*   ???????? `DELETE /api/admin/analytics/page-views/me` ???? PV/UV ??????????????????????????????


### 4.7 ???????????????????????????
*   DataInitializer ???????????????????????? sangui???dmin_user1???editor_user2 ????????????????????? BCrypt ????????????????????????????????????????????? 123456 ???????????????????????????????????????????????????????????? Chrome ?????????
*   ????????? pp.bootstrap.super-admin-password???pp.bootstrap.admin-password???pp.bootstrap.editor-password ????????????????????????????????????????????????????????????????????????????????????????????????pp.bootstrap.default-password (?????? Sg!2025#Blog!) ???????????????
*   ???????????????????????????????????????????????????????????????????????????????????????????????????????????? /admin/profile ?????????????????????????????????
