...
*   访问日志支持“页面类型”筛选：`pageType=ARTICLE|ROBOT|PAGE` 分别对应文章访问（`postId` 非空）、机器页面（列表“文章”列精确为 `robots.txt/sitemap.xml`）与普通页面（非文章且非机器页面）。其中当选择 `ROBOT` 时，前端会自动关闭 `excludeSystemPages` 以避免互斥冲突导致结果为空（后端也兼容 `SYSTEM/BOT` 作为 `ROBOT` 的别名）。
