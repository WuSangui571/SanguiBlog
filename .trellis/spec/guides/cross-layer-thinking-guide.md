...
| Backend AI provider DNS | `docker-compose.yml`, `docker-compose.prod.yml`, `docs/docker-deploy.md` | The `backend` service performs Spring AI embedding/provider calls, so repository Compose files must preserve the backend DNS fallback used in production (`223.5.5.5`, `119.29.29.29`, `8.8.8.8`). Deployment smoke docs must test provider DNS/reachability from the `backend` container, and unauthenticated provider `401`/`403` from `/models` is a successful network-reachability signal. |
...
| Bad | MySQL reports healthy from a socket-only init probe before `sanguiblog_db.sql` is complete, backend connects to host/local database config, `.env.example` contains real or fake default secrets, backend provider DNS is missing from repository Compose files, sitemap/robots fall through to SPA HTML, SSE is buffered by Nginx, upload directories remain root-owned `755` without a fail-fast startup error, or asset URL config causes duplicated `/uploads/uploads/...` requests. |
