...
- 修改类型：docs
- 影响范围：`release/` 中文发布说明、根目录英文 README、根目录中文 README、AI 修改日志
- 变更摘要：
  1) 检索确认当前站点版本主链路已经是 `V2.2.23`，但 `release/` 目录中仍缺少 `V2.2.23.md`，现有最新公开 release 文档仍停留在 `release/V2.2.20.md`。
  2) 复用 `release/V2.2.20.md` 的发布说明结构，新建 `release/V2.2.23.md`，按“版本概览 / 重点更新 / 部署影响 / 升级建议 / 兼容性 / 相关文档”组织内容，不另起第二套 release 模板。
  3) 基于 `.ai/CHANGELOG_AI.md` 中 `V2.2.20` 之后的真实变更，提炼首页文章区分阶段加载、首页手机端文章区体验优化、旧站 301 跳转来源识别、后台系统监控分组、文章封面上传护栏、桌面端后台侧栏折叠等对外可感知更新，避免把纯内部测试/留痕机械搬进 release 文档。
  4) 将 `README.md` 与 `README.zh-CN.md` 中“当前最新公开 release 文档”相关说明从 `V2.2.20` 同步更新为 `V2.2.23`，其余已有的 `V2.2.23` 版本说明和近期能力概述保持不变。
- 涉及文件：
  - `release/V2.2.23.md`
  - `README.md`
  - `README.zh-CN.md`
  - `.ai/CHANGELOG_AI.md`
- 检索与复用策略：
  - 检索关键词：`release/V2.2.20` / `release/V2.2.23` / `V2.2.23` / `系统监控` / `封面上传` / `后台左侧导航` / `文章区分阶段加载` / `redirect_from`
  - 候选实现：`release/V2.2.20.md` 既有 release 模板、`.ai/CHANGELOG_AI.md` 中 `V2.2.20` 之后的用户可感知改动、`README.md` 当前 release 引用说明、`README.zh-CN.md` 当前 release 引用说明
  - 最终选择：复用现有 release 文档结构和 README 组织方式，只补齐当前版本 release 文档并更新两处过时引用，不新增额外文档类型、不重写 README 主体内容
- 验证方式：
  - 执行 `rg -n "release/V2\\.2\\.20|release/V2\\.2\\.23" README.md README.zh-CN.md`，确认 README 引用已切到 `V2.2.23`
  - 执行 `git diff --check` 做静态格式检查
- 版本号说明：本次不再变更站点版本号；当前站点版本已是 `V2.2.23`，本轮仅补齐对应 release 文档并同步文档引用。
