...
- 背景/需求：后台 `/admin/analytics` 访问日志筛选区的起始日期/结束日期空值显示为浏览器原生本地化格式 `yyyy/mm/日`，与期望的 `yyyy/mm/dd` 不一致。
- 修改类型：fix
- 影响范围：后台访问日志筛选区日期输入框展示
- 变更摘要：
  1) 日期输入框在“空值且未聚焦”时改为文本占位显示 `yyyy/mm/dd`。
  2) 聚焦后自动切回原生 `date` 输入，保留系统日期选择器与原有查询参数格式 `yyyy-MM-dd`。
- 涉及文件：
...
- 检索与复用策略：
  ...
  - 最终选择：复用现有筛选状态与查询逻辑，仅在输入框显示层增加聚焦态切换，不新增接口或独立组件
- 风险点：
  - 仅调整空值展示与聚焦切换；若个别浏览器对动态切换 input type 兼容性较差，可能退化成普通文本框，但不会影响已输入值与查询参数结构。
- 验证方式：
  - 手动：打开 `/admin/analytics`，空值时看到 `yyyy/mm/dd`；点击后可正常弹出日期选择器；选择日期后筛选仍可生效。
