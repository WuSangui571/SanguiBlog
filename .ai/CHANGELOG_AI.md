...
- 变更摘要：
  ...
  9) 针对“马上点击 CTA 时仍有轻微卡顿”的反馈，确认根因是 `HomeView` 占位骨架与 `ArticleList` 挂载后的 `postsLoading` 卡片形成两段加载态；现改为点击/滚动启用前先通过现有 `onQueryChange({ page: 1, size })` 预取首页第一页，等初始加载结束后再显示真实 `ArticleList`，并通过 `skipInitialQuery` 让 `ArticleList` 首次挂载时跳过重复查询，减少从一套加载态切到另一套加载态的顿挫。
