...
- AI 助手支持“当前页面临时上下文增强”：当前仅在 `/article/{id}` 文章详情页启用，前端会把当前文章标题、摘要、正文节选、页面链接与图片数量线索随聊天请求一起发送；后端仅在用户问题明显指向“此页面/这篇博客/这篇文章/本文/当前文章”时使用这段上下文，不入向量库、不写数据库。图片数量线索只用于确认文章确实含配图，不代表 AI 已识别图片具体内容；回答时应避免“若有配图/虽未展示”这类与页面事实冲突的话术。
