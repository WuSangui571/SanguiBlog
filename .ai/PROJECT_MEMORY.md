...
*   **AI 助手前端入口**：自 V2.1.290 后，前台页面提供右下角 AI 助手悬浮入口；`/admin` 及其子页面始终隐藏该入口。自 2026-03-18 起，未登录用户前台也会显示 AI 助手入口，但首次发送消息只在前端提示“请先登录后使用”，不会触发建会话或聊天 API；登录用户才会进入真实 AI 会话、历史会话与流式回复链路。若用户从已登录切换为未登录，AI 面板会立即清空本地消息与会话状态，避免残留上一个登录用户的聊天记录。
*   **AI 助手后端接入（通义千问）**：后端基于 `Spring AI + Spring AI Alibaba DashScope` 调用通义千问，模型默认读取 `AI_DASHSCOPE_CHAT_MODEL`（缺省 `qwen-flash`），环境变量支持 `SPRING_AI_DASHSCOPE_API_KEY` 或 `AI_DASHSCOPE_API_KEY`。当前不再使用 `conversationId + JdbcChatMemoryRepository` 的自动恢复方案，而是改为业务侧会话表：
  ...
  * `GET /api/ai/sessions`：读取当前登录用户的历史会话列表。
  ...
  * 助手回复的 Markdown 根节点使用 `sg-ai-message-text`，并在全局 CSS 中显式启用 `user-select: text` / `-webkit-user-select: text`；后续调整 AI 消息样式时不要移除这层选择兜底，否则会导致助手回复拖选和右键复制不稳定。代码块复制按钮等交互控件需保持 `user-select: none`，避免按钮文本混入用户选区。AI 面板根层不应在任意 `pointerdown` 时直接调用 `raiseAssistantOverlay()`；当前通过 `aiSelectionGuard.js` 在右键、已有文本选区，或 pointer 目标本身就位于消息正文这类可选文本区域时跳过层级提升，避免首次从助手回复开始拖选时被 React 状态更新打断选区。消息正文容器需继续保留 `data-ai-message-selectable="true"` 标记，供该 guard 识别。
  * AI 助手 Markdown 代码块的复制交互当前统一复用 `src/appfull/ui/clipboardCopy.js` 的 `copyTextWithFallback(...)`：优先尝试 `navigator.clipboard.writeText`，若浏览器因权限、安全上下文或焦点原因拒绝，再自动降级到隐藏 `textarea + execCommand('copy')`。后续不要在 AI 代码块里再手写一套只走 Clipboard API 的复制分支，否则容易再次出现“按钮能点但实际未复制”的回归；复制成功/失败要保留可见状态反馈，但动效应保持克制。
  * AI 助手顶部会话区当前采用“图标工具栏 + 历史会话浮窗”形式：顶部仅保留“新对话”“历史会话”两个图标按钮，并与关闭按钮并列收口在头部右侧；历史会话点击后以浮窗列表展示，列表项显示会话标题截断与相对时间，不再使用横向滚动会话条。
  * 当前若 AI 面板正处于空白新会话状态（尚未发送首条消息），头部“新对话”按钮会置灰并禁用；历史会话浮窗支持右上角关闭按钮与点击外部关闭。
  * 头部右侧当前共四个图标按钮：新对话、历史会话、浮动窗口、关闭。浮动窗口模式为纯前端临时态：开启后 AI 面板脱离右侧停靠并可拖动，再次点击恢复停靠；关闭面板或下次重新打开时均回到默认右侧位置，不保留上一次的浮动状态和坐标。
  * 浮动窗口当前仅允许通过顶部标题栏拖动；顶部标题栏显示拖拽手势，正文区域不再显示拖拽手势或响应拖拽，避免“看起来能拖但实际拖不动”的交互错位。
  * 当历史会话浮窗打开时，AI 主聊天滚动区会被临时锁定，正文区域铺设透明交互锁层；此时仅允许滚动历史会话列表，关闭历史浮窗后恢复主聊天区滚动。
...
*   自 V1.3.106 起，主题切换按钮支持“超频模式”彩蛋：450ms 内连续点击 ≥6 次会触发 15 秒全屏矩阵光效与提示；V1.3.107 起冷却提示在超频展示期间自动抑制，避免消息互相覆盖。自 V2.2.20 起，顶部主题超频/冷却提示也改为复用 `GlassPopupToast` 玻璃模板，和站点其余前台短提示保持一致；矩阵光效背景本体继续保留。相关阈值仍由 `THEME_SPREE_THRESHOLD`、`THEME_SPREE_DURATION`、`THEME_LOCK_DURATION` 控制。
...
*   自 V2.2.19 起，首页 Hero 的“向下探索内容”按钮在手机端不再依赖 `home-status-strip` 作为落点，而是优先滚动到首篇文章锚点 `#home-first-post`，从而跳过首页“文章搜索”卡片；桌面端仍保持原有 `home-status-strip -> #posts` 的滚动链路。后续若再调整首页结构，请优先保留这个首篇文章锚点，而不是把手机端 CTA 重新绑回状态条。
*   自 2026-04-15 起，首页采用“Hero 首屏优先、文章区分阶段加载”策略：`HomeView.jsx` 中 `ArticleList` 使用 `React.lazy` 拆出，初始渲染 `HomeArticleListPlaceholder` 作为文章区同款轻量骨架；用户滚动、文章区接近视口、首屏动画后浏览器空闲，或点击 Hero CTA 时才启用真实 `ArticleList`。文章数据仍复用现有 `ArticleList -> onQueryChange(loadPosts) -> fetchPosts('/posts')` 分页链路，不新增第二套首页或文章接口。若 CTA 点击时真实文章区尚未下载/渲染完成，应先滚到始终挂载的 `home-deferred-posts-anchor` 稳定锚点，并通过 `heroCtaScrollInProgressRef` 在约 720ms 的平滑滚动窗口内屏蔽滚动监听、IntersectionObserver 与 idle 预热触发，等滚动稳定后再启用文章区加载；不要把滚动目标绑在会被 lazy fallback/真实列表切换卸载的节点上，也不要排队二次跳到 `#home-first-post`，避免“刚点击就半路停下”或“先跳一半、加载后再跳一次”的割裂感。延迟文章区的占位高度不能过短，当前 `HomeArticleListPlaceholder` 需要至少预留一屏高度（`min-h-screen`），否则页面总高度不足时浏览器没有足够可滚动空间，Hero CTA 会被迫停在半路。占位视觉不要再使用独立的 `Articles Preparing` 临时卡，应复用 `StatsStrip` 并呈现“文章搜索 + 文章加载中…”等接近真实文章区的骨架，避免用户看到两套割裂 UI。为减少“占位骨架 -> ArticleList 自身 postsLoading 卡片”的二次加载态切换，`HomeView` 会在启用真实列表前先用现有 `onQueryChange({ page: 1, size })` 预取首页第一页，并且只有观察到本次请求进入过 `postsLoading=true` 再回到 `false` 后，才渲染真实 `ArticleList`；预取过的场景必须向 `ArticleList` 传 `skipInitialQuery`，且 `ArticleList` 需要将该跳过标记延迟到下一轮事件循环再释放，兼容开发环境 `React.StrictMode` 双 effect，避免真实文章卡短暂出现后又被第二次自动查询打回 loading。
...
*   自 V2.1.275 起，目录浮层加入 ScrollSpy：滚动阅读时自动高亮当前所在章节，并在目录滚动区域自动跟随定位（桌面侧栏与移动抽屉同步生效），降低长文查找成本。
...
*   访问日志列表的“隐藏 robots/sitemap”采用服务端分页过滤：前端会在 `GET /api/admin/analytics/page-views` 附加 `excludeSystemPages=true`，后端会以列表“文章”列的展示值为准（`post.title` 优先，否则 `page_title`），仅当该列精确为 `robots.txt` 或 `sitemap.xml` 时才过滤，以减少搜索引擎/爬虫抓取噪音并保证分页/total 准确；点击“重置”会同时清空查询条件并取消该隐藏开关。

*   访问日志支持“页面类型”筛选：`pageType=ARTICLE|ROBOT|PAGE` 分别对应文章访问（以 `analytics_page_views.post_id` 是否为空为准，最直观且稳定）、机器页面（列表“文章”列精确为 `robots.txt/sitemap.xml`）与普通页面（非文章且非机器页面）。其中当选择 `ROBOT` 时，前端会自动关闭 `excludeSystemPages` 以避免互斥冲突导致结果为空（后端也兼容 `SYSTEM/BOT` 作为 `ROBOT` 的别名）。
...
- “最新/最近/多少篇/排序”这类结构化事实问题不应交给纯 RAG 推断；当前优先由 `AiAssistantCapabilityService` 直接查询 MySQL 或返回保守引导文案，避免模型编造不存在的文章标题。
- AI 助手支持“当前页面临时上下文增强”：当前仅在 `/article/{id}` 文章详情页启用，前端会把当前文章标题、摘要、正文节选、页面链接与正文图片数量线索随聊天请求一起发送；后端仅在用户问题明显指向“此页面/这篇博客/这篇文章/本文/当前文章”时使用这段上下文，不入向量库、不写数据库。图片数量线索只统计文章正文中的 Markdown 图片和 HTML `<img>`，不统计文章封面图；该线索只用于确认文章确实含配图，不代表 AI 已识别图片具体内容。回答时应避免“若有配图/虽未展示”这类与页面事实冲突的话术；若用户要求讲解/识别配图或图片内容，应明确说明当前暂未实装图片识别功能，只能确认正文图片数量，不能编造图中细节。
- AI 助手支持“当前登录用户身份上下文”：后端会在每轮已登录聊天请求中，把真实用户名、显示名、头衔、角色名和角色编码加入 system prompt，用于稳定回答“我是谁/我当前是什么角色”等问题；这层信息不依赖前端传参。
...
- AI 浮动窗口模式支持四边与四角拉伸，拉伸后的宽高只在当前浮动态临时生效；退出浮动模式后必须恢复默认右侧停靠位置与默认尺寸。
