...
*   自 2026-04-15 起，首页采用“Hero 首屏优先、文章区分阶段加载”策略：`HomeView.jsx` 中 `ArticleList` 使用 `React.lazy` 拆出，初始只渲染轻量 `#posts` 占位；用户滚动、文章区接近视口、首屏动画后浏览器空闲，或点击 Hero CTA 时才启用真实 `ArticleList`。文章数据仍复用现有 `ArticleList -> onQueryChange(loadPosts) -> fetchPosts('/posts')` 分页链路，不新增第二套首页或文章接口。移动端若 CTA 点击时首篇文章尚未渲染，会在文章数据就绪后继续滚到 `#home-first-post`，后续调整时不要破坏这条兜底。
