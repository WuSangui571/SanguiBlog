...
*   自 2026-04-15 起，首页采用“Hero 首屏优先、文章区分阶段加载”策略：`HomeView.jsx` 中 `ArticleList` 使用 `React.lazy` 拆出，初始只渲染轻量 `home-status-strip/#posts` 占位；用户滚动、文章区接近视口、首屏动画后浏览器空闲，或点击 Hero CTA 时才启用真实 `ArticleList`。文章数据仍复用现有 `ArticleList -> onQueryChange(loadPosts) -> fetchPosts('/posts')` 分页链路，不新增第二套首页或文章接口。若 CTA 点击时真实文章区尚未下载/渲染完成，应先滚到始终挂载的 `home-deferred-posts-anchor` 稳定锚点，并通过 `heroCtaScrollInProgressRef` 在约 720ms 的平滑滚动窗口内屏蔽滚动监听、IntersectionObserver 与 idle 预热触发，等滚动稳定后再启用文章区加载；不要把滚动目标绑在会被 lazy fallback/真实列表切换卸载的节点上，也不要排队二次跳到 `#home-first-post`，避免“刚点击就半路停下”或“先跳一半、加载后再跳一次”的割裂感。
